...
/// for the ladder tests. `joinedAt` is pinned at "long ago" so the
/// `firstNight` branch isn't accidentally hit by a 0-sessions member.
private func openerTestMember(
    userId: UUID,
    displayName: String,
...
/// Helper: a claimed RSVP for the member.
private func openerTestRsvp(
    eventId: UUID,
    memberId: UUID,
    displayName: String,
    state: MemberRSVPState = .claimed
) -> EventRSVP {
    EventRSVP(
        eventId: eventId,
        memberId: memberId,
...
/// Helper: a leaderboard row with the supplied sessionsPlayed +
/// lastSessionDelta so the `firstNight`, `lastWinner`, and
/// `longTenure` branches are individually reachable.
private func openerTestLeaderboard(
    userId: UUID,
    displayName: String,
...
runner.run("HostOpenerDerivation (a) mirrors the member's socialText, name-first") {
    let eventId = UUID()
    let userId = UUID()
...
runner.run("HostOpenerDerivation (b) opens on conversationPrompt when no socialText") {
    ...
    let member = openerTestMember(
        ...
        socialPreference: SocialPreference(
            socialText: "",
            conversationPrompt: "Tell me your favourite film",
            defaultSet: true
        )
    )
    let rsvp = openerTestRsvp(eventId: eventId, memberId: userId, displayName: "Sam")
    let board = [openerTestLeaderboard(userId: userId, displayName: "Sam", sessionsPlayed: 3, lastSessionDelta: 0)]
    let line = HostOpenerDerivation.suggestion(
        eventId: eventId, member: member, rsvp: rsvp,
        leaderboardEntry: board[0], now: Date()
    )
    runner.assertEqual(line.branch, .conversationPrompt)
    runner.assertTrue(line.line.contains("Sam"), "name-first")
    runner.assertTrue(line.line.contains("favourite film"), "topic surfaced")
}

runner.run("HostOpenerDerivation (c) welcomes a first-night member by name, no history") {
    let eventId = UUID()
    let userId = UUID()
    let member = openerTestMember(userId: userId, displayName: "Robin")
    let rsvp = openerTestRsvp(eventId: eventId, memberId: userId, displayName: "Robin")
    let board = [openerTestLeaderboard(userId: userId, displayName: "Robin", sessionsPlayed: 0, lastSessionDelta: 0)]
    let line = HostOpenerDerivation.suggestion(
        eventId: eventId, member: member, rsvp: rsvp,
        leaderboardEntry: board[0], now: Date()
    )
    runner.assertEqual(line.branch, .firstNight)
    runner.assertTrue(line.line.contains("Robin"), "name surfaced")
    runner.assertTrue(line.line.lowercased().contains("first night"), "first-night tag")
}

runner.run("HostOpenerDerivation (d) plays the last-win reputation when lastSessionDelta > 0") {
    let eventId = UUID()
    let userId = UUID()
    let member = openerTestMember(userId: userId, displayName: "Casey")
    let rsvp = openerTestRsvp(eventId: eventId, memberId: userId, displayName: "Casey")
    let board = [openerTestLeaderboard(userId: userId, displayName: "Casey", sessionsPlayed: 4, lastSessionDelta: 60)]
    let line = HostOpenerDerivation.suggestion(
        eventId: eventId, member: member, rsvp: rsvp,
        leaderboardEntry: board[0], now: Date()
    )
    runner.assertEqual(line.branch, .lastWinner)
    runner.assertTrue(line.line.contains("Casey"), "name surfaced")
    runner.assertTrue(line.line.lowercased().contains("last"), "reputation hint")
}

runner.run("HostOpenerDerivation (e) plays the tenure line when sessionsPlayed >= 5 and no win to defend") {
    let eventId = UUID()
    let userId = UUID()
    let member = openerTestMember(userId: userId, displayName: "Drew")
    let rsvp = openerTestRsvp(eventId: eventId, memberId: userId, displayName: "Drew")
    let board = [openerTestLeaderboard(userId: userId, displayName: "Drew", sessionsPlayed: 7, lastSessionDelta: -30)]
    let line = HostOpenerDerivation.suggestion(
        eventId: eventId, member: member, rsvp: rsvp,
        leaderboardEntry: board[0], now: Date()
    )
    runner.assertEqual(line.branch, .longTenure)
    runner.assertTrue(line.line.contains("Drew"), "name surfaced")
    runner.assertTrue(line.line.lowercased().contains("five") || line.line.lowercased().contains("tempo"),
                      "tenure copy surfaced")
}

runner.run("HostOpenerDerivation (f) falls back to a name-forward greeting") {
    let eventId = UUID()
    let userId = UUID()
    let member = openerTestMember(userId: userId, displayName: "Pat")
    let rsvp = openerTestRsvp(eventId: eventId, memberId: userId, displayName: "Pat")
    let board = [openerTestLeaderboard(userId: userId, displayName: "Pat", sessionsPlayed: 2, lastSessionDelta: 0)]
    let line = HostOpenerDerivation.suggestion(
        eventId: eventId, member: member, rsvp: rsvp,
        leaderboardEntry: board[0], now: Date()
    )
    runner.assertEqual(line.branch, .nameForward)
    runner.assertTrue(line.line.contains("Pat"), "name surfaced")
}

runner.run("HostOpenerDerivation (a) is NOT hit when socialText is the seeded default") {
    // The runner seeds new members with `SocialPreference.defaultSocialText`;
    // mirroring it back would read as "the system says you're shy" —
    // not a host move. The ladder must skip branch (a) and fall through.
    let eventId = UUID()
    let userId = UUID()
    let member = openerTestMember(
        userId: userId,
        displayName: "Jamie",
        socialPreference: SocialPreference(
            socialText: SocialPreference.defaultSocialText,
            conversationPrompt: "",
            defaultSet: true
        )
    )
    let rsvp = openerTestRsvp(eventId: eventId, memberId: userId, displayName: "Jamie")
    let board = [openerTestLeaderboard(userId: userId, displayName: "Jamie", sessionsPlayed: 2, lastSessionDelta: 0)]
    let line = HostOpenerDerivation.suggestion(
        eventId: eventId, member: member, rsvp: rsvp,
        leaderboardEntry: board[0], now: Date()
    )
    runner.assertTrue(line.branch != .memberStatedPreference, "seeded default must skip branch (a)")
}

runner.run("HostOpenerDerivation (a) is NOT hit when socialText is empty or whitespace") {
    let eventId = UUID()
    let userId = UUID()
    let member = openerTestMember(
        userId: userId,
    ...
    let rsvp = openerTestRsvp(eventId: eventId, memberId: userId, displayName: "Lee")
    let board = [openerTestLeaderboard(userId: userId, displayName: "Lee", sessionsPlayed: 2, lastSessionDelta: 0)]
    let line = HostOpenerDerivation.suggestion(
        eventId: eventId, member: member, rsvp: rsvp,
        leaderboardEntry: board[0], now: Date()
    )
    runner.assertTrue(line.branch != .memberStatedPreference, "whitespace must skip branch (a)")
}

runner.run("HostOpenerDerivation.suggestions filters to claimed RSVPs only") {
    let eventId = UUID()
    let claimedId = UUID()
    let declinedId = UUID()
    let unclaimedId = UUID()
    let members = [
        openerTestMember(userId: claimedId, displayName: "Claimed"),
        openerTestMember(userId: declinedId, displayName: "Declined"),
        openerTestMember(userId: unclaimedId, displayName: "Unclaimed")
    ]
    let rsvps = [
        openerTestRsvp(eventId: eventId, memberId: claimedId, displayName: "Claimed", state: .claimed),
        openerTestRsvp(eventId: eventId, memberId: declinedId, displayName: "Declined", state: .declined),
        openerTestRsvp(eventId: eventId, memberId: unclaimedId, displayName: "Unclaimed", state: .unclaimed)
    ]
    let lines = HostOpenerDerivation.suggestions(
        eventId: eventId, members: members, rsvps: rsvps,
        leaderboard: [], now: Date()
    )
    runner.assertEqual(lines.count, 1)
    runner.assertEqual(lines.first?.displayName, "Claimed")
    runner.assertEqual(lines.first?.branch, .nameForward)
}

runner.run("HostOpenerDerivation.suggestions synthesises a fallback member when cached members omit the row") {
    // Stale-cache scenario: the leaderboard / RSVP cache carries a
    // claimed seat for a member who hasn't shown up in `cachedMembers`
    // yet. The derivation must NOT skip the row — every claimed seat
    // gets exactly one opener.
    let eventId = UUID()
    let claimedId = UUID()
    let rsvps = [openerTestRsvp(eventId: eventId, memberId: claimedId, displayName: "Ghost")]
    let lines = HostOpenerDerivation.suggestions(
        eventId: eventId, members: [], rsvps: rsvps,
        leaderboard: [], now: Date()
    )
    runner.assertEqual(lines.count, 1)
    runner.assertEqual(lines.first?.displayName, "Ghost")
}

runner.run("HostOpenerDerivation is deterministic — same inputs produce same line") {
    let eventId = UUID()
    let userId = UUID()
    ...
    let member = openerTestMember(
        ...
        displayName: "Toni",
        socialPreference: SocialPreference(
            socialText: "I like to warm up slowly",
            conversationPrompt: "",
            defaultSet: true
        )
    )
    let rsvp = openerTestRsvp(eventId: eventId, memberId: userId, displayName: "Toni")
    let board = [openerTestLeaderboard(userId: userId, displayName: "Toni", sessionsPlayed: 4, lastSessionDelta: 0)]
    let first = HostOpenerDerivation.suggestion(
        eventId: eventId, member: member, rsvp: rsvp,
        leaderboardEntry: board[0], now: now
    )
    let second = HostOpenerDerivation.suggestion(
        eventId: eventId, member: member, rsvp: rsvp,
        leaderboardEntry: board[0], now: now
    )
    runner.assertEqual(first.line, second.line)
    runner.assertEqual(first.branch, second.branch)
    runner.assertEqual(first.id, second.id)
}

runner.run("HostOpenerDerivation stays <= 140 chars even when socialText is very long") {
    let eventId = UUID()
    let userId = UUID()
    ...
    let rsvp = openerTestRsvp(eventId: eventId, memberId: userId, displayName: "Verbose")
    let board = [openerTestLeaderboard(userId: userId, displayName: "Verbose", sessionsPlayed: 3, lastSessionDelta: 0)]
    let line = HostOpenerDerivation.suggestion(
        eventId: eventId, member: member, rsvp: rsvp,
        leaderboardEntry: board[0], now: Date()
    )
...
runner.run("HostOpenerSuggestion id is stable composite of event + member") {
    let eventId = UUID()
    let memberId = UUID()
    let rsvp = EventRSVP(eventId: eventId, memberId: memberId, displayName: "X", state: .claimed)
    let member = openerTestMember(userId: memberId, displayName: "X")
    let line = HostOpenerDerivation.suggestion(
        eventId: eventId, member: member, rsvp: rsvp,
        leaderboardEntry: nil, now: Date()
    )
